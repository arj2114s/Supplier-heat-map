...
# ==========================================
# 2. WEIGHT CONFIGURATION (MAIN PAGE)
# ==========================================
st.header("⚙️ 1. Weight Configuration (%)")

st.subheader("Likelihood Weights")
lc1, lc2, lc3, lc4 = st.columns(4)
w_ncr = lc1.number_input("NCR Rate Weight", 0, 100, 40)
w_resch = lc2.number_input("Reschedule Rate Weight", 0, 100, 25)
w_past_ch = lc3.number_input("Past Changes Weight", 0, 100, 20)
w_resp = lc4.number_input("Responsiveness Weight", 0, 100, 15)

st.subheader("Impact Weights")
ic1, ic2, ic3, ic4 = st.columns(4)
w_avg_delay = ic1.number_input("Avg delay days Weight", 0, 100, 15)
w_max_delay = ic2.number_input("Max Delay Days Weight", 0, 100, 15)
w_otd = ic3.number_input("OTD % Weight", 0, 100, 20)
w_ncr_close = ic4.number_input("NCR closure time Weight", 0, 100, 10)

ic5, ic6, ic7, ic8 = st.columns(4)
w_insp = ic5.number_input("Inspection cycle Weight", 0, 100, 10)
w_wo_cr = ic6.number_input("Site WO-CR Weight", 0, 100, 10)
w_ep_cr = ic7.number_input("E&P-CR Weight", 0, 100, 10)
w_po_cr = ic8.number_input("PO-CR Weight", 0, 100, 10)
...
# ==========================================
# 3. SUPPLIER DATA EDITOR
# ==========================================
st.header("📝 2. Edit Supplier Scores (1-5)")
...
st.caption(f"Currently editing parameters for **{selected_supplier}**:")
...
with tab1:
    ...
    # Background Shapes for Professional Heat Map Zones
    fig.add_shape(type="rect", x0=1, y0=1, x1=2.5, y1=2.5, fillcolor="rgba(76, 175, 80, 0.4)", opacity=1, layer="below", line_width=0)
    fig.add_shape(type="rect", x0=2.5, y0=1, x1=5, y1=2.5, fillcolor="rgba(255, 152, 0, 0.4)", opacity=1, layer="below", line_width=0)
    fig.add_shape(type="rect", x0=1, y0=2.5, x1=2.5, y1=5, fillcolor="rgba(255, 152, 0, 0.4)", opacity=1, layer="below", line_width=0)
    fig.add_shape(type="rect", x0=2.5, y0=2.5, x1=5, y1=5, fillcolor="rgba(244, 67, 54, 0.4)", opacity=1, layer="below", line_width=0)
